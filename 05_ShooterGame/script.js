...
class Explosion {
  ...
  draw() {
    context.drawImage(this.image, this.frame * this.spriteWidth, 0, this.spriteWidth, this.spriteHeight, this.x, this.y - this.size * 0.25, this.size, this.size);
  }
};
...
function drawScore() {
  ...
  context.fillText('Score: ' + score, 55, 75);
};
...
function drawGameOver() {
  ...
  context.fillStyle = 'white';
  context.fillText('GAME OVER, your score is ' + score, canvas.width * 0.5 + 5, canvas.height * 0.5 + 5);
};

window.addEventListener('click', function (e) {
  // getImageData(width, height, areaWidth, areaHeight)
...
function animate(timestamp) {
  ...
  if (timeToNextRaven > ravenInterval) {
    ...
    timeToNextRaven = 0;
    // sort ravens in array in order by their size, larger ones in front
  ...
  particles = particles.filter(particles => !particles.markedForDeletion);
